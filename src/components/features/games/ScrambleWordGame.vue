<template>
  <div class="bg-white p-6 rounded-2xl shadow-lg">
    <h2 class="text-2xl font-bold text-center mb-6 text-gray-800">🔤 Kata Acak!</h2>
    
    <!-- Word Display -->
    <div class="text-center mb-6">
      <div 
        class="bg-gradient-to-r from-orange-100 to-red-100 p-6 rounded-xl border-2 border-dashed border-orange-300 transition-all duration-300 hover:shadow-lg"
      ...
      >
        ...
        <p 
          class="text-3xl font-bold text-orange-600 tracking-widest transition-all duration-500"
    ...
    <form @submit.prevent="checkAnswer" class="space-y-4">
      ...
      <button
        v-if="!showResult"
        type="submit"
        class="w-full bg-gradient-to-r from-orange-500 to-red-500 text-white py-3 px-6 rounded-lg font-semibold hover:from-orange-600 hover:to-red-600 transform hover:scale-105 transition-all duration-200 shadow-lg"
    ...
    <div v-if="showResult" class="mt-6 text-center">
      ...
      >
        ...
        <div v-if="!isCorrect && currentWord" class="mt-2 text-sm">
          <p>Jawaban yang benar: <strong>{{ currentWord.answer }}</strong></p>
        </div>
      </div>
