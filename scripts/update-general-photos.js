...
async function getGeneralPhotosFromCloudinary() {
  try {
    ...
    try {
      const nonFormalResult = await cloudinary.api.resources_by_asset_folder(
        folderName,
        {
          max_results: 500,
        }
      ...
      log(
        `📁 ${folderName}: ${nonFormalPhotos.length} photos ditemukan`,
        "success"
      );
      ...
      log(
        `✅ Berhasil mengambil ${nonFormalPhotos.length} foto untuk generalphoto`,
        "success"
      );
      ...
      log(
        `⚠️ Error menggunakan resources_by_asset_folder: ${err.message}`,
        "warning"
      );
      ...
      log(
        `📁 nonformal-T25 (fallback): ${fallbackPhotos.length} photos ditemukan`
      );
...
function extractNameFromFilename(publicId) {
  ...
  const words = cleanName
    ...
    .filter(
      (word) =>
        word.length > 1 && !/^\d+$/.test(word) && !/^[a-z0-9]{6,}$/i.test(word)
    );
...
function calculateNameSimilarity(name1, name2) {
  ...
  return {
    ...
    isPartialMatch:
      matchedWords.length >= Math.min(2, words2.length) && similarity >= 0.5,
  };
}
...
async function matchPhotosWithStudents(photos, students) {
  ...
  if (unmatchedPhotos.length > 0) {
    log(
      `🔍 Mencoba matching ${unmatchedPhotos.length} foto berdasarkan nama...`
    );
    ...
    unmatchedPhotos.forEach((photo) => {
      ...
      if (extractedName && extractedName.length > 2) {
        ...
        students.forEach((student) => {
          ...
          const similarityResult = calculateNameSimilarity(
            fullName,
            extractedName
          );

          if (
            similarityResult.isPartialMatch &&
            similarityResult.similarity > bestScore
          ) {
            bestMatch = {
              student: student,
    ...
    if (nameMatches.length > 0) {
      ...
      nameMatches.forEach((match, index) => {
        console.log(
          `${colors.white}${index + 1}. ${match.photo.filename}${colors.reset}`
        );
        console.log(
          `   👤 Matched: ${match.match.student.fullName || match.match.student.name} (${match.nim})`
        );
        ...
        console.log(
          `   🎯 Matched words: ${match.match.matchedWords.join(", ")}`
        );
        console.log();
      });
    }
  ...
  log(
    `✅ Total berhasil mencocokkan ${totalMatched} foto dari ${photos.length} foto (${nimMatchedCount} by NIM, ${nameMatchedCount} by name)`,
    "success"
  );
  ...
  const finalUnmatched = photos.filter((photo) => {
    ...
    const hasNameMatch = Object.values(matches).some(
      (match) => match.photo.public_id === photo.public_id
    );
...
async function updateGeneralPhotosInJSON(matches) {
  try {
    ...
    log(
      `✅ Berhasil mengupdate ${updatedCount} properti generalphoto`,
      "success"
    );
...
async function showSummary(matches) {
  ...
  console.log(
    `${colors.white}Total foto ditemukan:     ${Object.keys(matches).length}${colors.reset}`
  );
  console.log(
    `${colors.green}Mahasiswa yang diupdate:  ${Object.keys(matches).length}${colors.reset}`
  );

  console.log(`\n${colors.magenta}=== DETAIL MATCHING ===${colors.reset}`);
  Object.entries(matches).forEach(([nim, match], index) => {
    console.log(
      `${colors.white}${index + 1}. ${match.student.fullName || match.student.name} (${nim})${colors.reset}`
    );
...
async function main() {
  console.log(
    `${colors.green}🖼️  General Photos Updater (T-25-nonFormal)${colors.reset}`
  );
  console.log(
  ...
  try {
    ...
    if (Object.keys(matches).length === 0) {
      log(
        "❌ Tidak ada foto yang berhasil dicocokkan. Silakan periksa format filename atau folder.",
        "error"
      );
      return;
    }
    ...
    const confirm = await new Promise((resolve) => {
      rl.question(
        `\n${colors.yellow}Apakah Anda yakin ingin mengupdate properti generalphoto di people.json? (y/N): ${colors.reset}`,
        resolve
      );
    });
    ...
    if (confirm.toLowerCase() === "y" || confirm.toLowerCase() === "yes") {
      ...
      log(
        `🎉 Selesai! ${updatedCount} generalphoto berhasil diupdate dari folder nonformal-T25`,
        "success"
      );
    } else {
...
async function interactiveRenameAssets({ dryRun = false } = {}) {
  ...
  if (selectedAssets.length === 1) {
    ...
    for (let i = 0; i < selectedAssets.length; ++i) {
      let base = await ask(
        `Nama baru untuk [${selectedAssets[i].public_id}]: `
      );
      base = base.trim();
      if (!base) {
        log("Nama baru tidak boleh kosong.", "error");
    ...
    try {
      const exists = await cloudinary.api
        ...
        .catch((e) => {
          if (
            e &&
            (e.http_code === 404 || (e.error && e.error.http_code === 404))
          )
            return false;

          log(
            `Gagal cek public_id ${newId}: ${e && e.message ? e.message : JSON.stringify(e)}`,
            "error"
          );
